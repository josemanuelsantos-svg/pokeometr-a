...
const generateQuestion = (difficulty) => {
  ...
  if (difficulty === 1) {
    // Fácil: Áreas básicas, volúmenes básicos y conversiones simples
    availableTypes = ['area_cuadrado', 'area_rectangulo', 'volumen_cubo', 'volumen_prisma', 'conversion_simple'];
  } else if (difficulty === 2) {
    // Medio: Áreas intermedias, volúmenes intermedios y equivalencias de capacidad
    availableTypes = ['area_triangulo', 'area_paralelogramo', 'volumen_cilindro', 'volumen_piramide', 'equivalencia_capacidad'];
  } else {
    // Difícil: Áreas complejas (círculo), volúmenes complejos y equivalencias de masa
    availableTypes = ['area_circulo', 'volumen_cono', 'volumen_esfera', 'equivalencia_masa'];
  }
  ...
  switch (type) {
    // --- ÁREAS ---
    case 'area_cuadrado':
      const l_cuad = Math.floor(Math.random() * 8) + 2;
      questionText = `Calcula el área de un cuadrado de lado ${l_cuad} cm.`;
      correctAnswer = l_cuad * l_cuad;
      unit = 'cm²';
      formulaInfo = 'A = Lado × Lado';
      break;
    case 'area_rectangulo':
      const b_rect = Math.floor(Math.random() * 8) + 3;
      const h_rect = Math.floor(Math.random() * 6) + 2;
      questionText = `Calcula el área de un rectángulo con base ${b_rect} cm y altura ${h_rect} cm.`;
      correctAnswer = b_rect * h_rect;
      unit = 'cm²';
      formulaInfo = 'A = Base × Altura';
      break;
    case 'area_triangulo':
      const b_tri = Math.floor(Math.random() * 10) + 4;
      const h_tri = Math.floor(Math.random() * 8) + 4;
      const basePar = b_tri % 2 === 0 ? b_tri : b_tri + 1; // Para evitar decimales
      questionText = `Calcula el área de un triángulo con base ${basePar} cm y altura ${h_tri} cm.`;
      correctAnswer = (basePar * h_tri) / 2;
      unit = 'cm²';
      formulaInfo = 'A = (Base × Altura) / 2';
      break;
    case 'area_paralelogramo':
      const b_para = Math.floor(Math.random() * 10) + 4;
      const h_para = Math.floor(Math.random() * 8) + 3;
      questionText = `Calcula el área de un paralelogramo con base ${b_para} cm y altura ${h_para} cm.`;
      correctAnswer = b_para * h_para;
      unit = 'cm²';
      formulaInfo = 'A = Base × Altura';
      break;
    case 'area_circulo':
      const r_circ = Math.floor(Math.random() * 6) + 3;
      questionText = `Calcula el área de un círculo de radio ${r_circ} cm (usa pi = 3.14).`;
      correctAnswer = roundToTwo(PI * r_circ * r_circ);
      unit = 'cm²';
      formulaInfo = 'A = π × r²';
      break;
  ...
  while (options.size < 4) {
    ...
    let randomFactor = Math.floor(Math.random() * (correctAnswer > 100 ? 50 : 10)) + baseOffset;
...
const HEROES = [
  { 
    id: 'ironman', name: 'Iron Man', img: 'https://i.ibb.co/27K5dCBM/b751779a4a3bbc38f9268036cdb5af5a.gif', fallback: '🤖', color: 'border-red-500',
    attacks: [
  ...
  { 
    id: 'cap', name: 'Capitán América', img: 'https://i.ibb.co/XqT34sz/189868-C0-D40619-AD55-4-B4-C-BE57-9005-D2506967-0-1643400842.gif', fallback: '🛡️', color: 'border-blue-500',
    attacks: [
  ...
  { 
    id: 'thor', name: 'Thor', img: 'https://i.ibb.co/PsFhhF1g/f604e46c6979b173d319fc064ed5c0dc.gif', fallback: '⚡', color: 'border-yellow-400',
    attacks: [
  ...
  { 
    id: 'widow', name: 'Viuda Negra', img: 'https://i.ibb.co/JjJQnWcH/0c2a5632830679-569563b0d45b2.gif', fallback: '🕷️', color: 'border-gray-500',
    attacks: [
  ...
  { 
    id: 'strange', name: 'Dr. Strange', img: 'https://i.ibb.co/M5VX25W0/tumblr-n11ui8-Bh-NU1r8bj4ko1-500.gif', fallback: '👁️', color: 'border-orange-500',
    attacks: [
  ...
  { 
    id: 'hulk', name: 'Hulk', img: 'https://i.ibb.co/BV1dZJCH/tumblr-nkx9ln-Ha8c1tiwiyxo1-640.gif', fallback: '💪', color: 'border-green-500',
    attacks: [
...
const VILLAINS = [
  { name: 'Ultron', hp: 250, maxHp: 250, level: 1, img: 'https://i.ibb.co/tpVWKW0d/marvel-future-fight-infinity-ultron.gif', fallback: '🦾', bgImg: 'https://images.unsplash.com/photo-1505506874110-6a7a4c9d2433?auto=format&fit=crop&q=80&w=1000', trait: 'Robot: Combate estándar.' },
  { name: 'Loki', hp: 500, maxHp: 500, level: 2, img: 'https://i.ibb.co/Ps65bscK/loki.gif', fallback: '🐍', bgImg: 'https://images.unsplash.com/photo-1462331940025-496dfbfc7564?auto=format&fit=crop&q=80&w=1000', trait: 'Ilusiones: Inmune a Ataques Fáciles.' },
  { name: 'Doctor Doom', hp: 900, maxHp: 900, level: 3, img: 'https://i.ibb.co/67dZ8jpD/doctor-doom-transparent-marvel-comics.gif', fallback: '🦹', bgImg: 'https://images.unsplash.com/photo-1509904297379-379e436f5193?auto=format&fit=crop&q=80&w=1000', trait: 'Escudo Mágico: Reduce Ataques Medios.' },
  { name: 'Thanos', hp: 1500, maxHp: 1500, level: 4, img: 'https://i.ibb.co/7NjPsfgb/183d8eefe6fe041dd1169fdeaab016f8.gif', fallback: '🧤', bgImg: 'https://images.unsplash.com/photo-1462332420958-a05d1e002413?auto=format&fit=crop&q=80&w=1000', trait: 'Titán Loco: Recibe Daño x2 de Ataques Definitivos.' },
];
...
const Avatar = ({ src, alt, fallback, className, isEnemy }) => {
  ...
  return (
    <div className="relative flex flex-col items-center justify-end">
      ...
      <div className={`absolute bottom-0 w-[120%] h-[30%] rounded-[100%] z-0 blur-[2px] ${isEnemy ? 'bg-red-900/60' : 'bg-cyan-900/60'} border-b-2 ${isEnemy ? 'border-red-500' : 'border-cyan-400'}`}></div>
      
      {imgError || !src ? (
        <div className={`flex items-center justify-center bg-slate-800 text-4xl shadow-inner relative z-10 ${className}`}>
          {fallback}
        </div>
      ) : (
        <img 
...
function MarvelMathGame() {
  ...
  const [currentAttackInfo, setCurrentAttackInfo] = useState({ name: '', damage: 0, diff: 1, isUltimate: false });
  ...
  const initBattle = (hero) => {
    ...
    setGameState('BATTLE');
    setMessages([`¡Adelante, ${hero.name}!`, `¡Un ${VILLAINS[0].name} salvaje apareció!`]);
  };
  ...
  const handleAttackClick = (difficulty, attackName, baseDamage, isUltimate = false) => {
    if (!isPlayerTurn) return;
    setCurrentAttackInfo({ name: attackName, damage: baseDamage, diff: difficulty, isUltimate });
  ...
  const handleAnswer = (selectedOption) => {
    ...
    if (selectedOption === question.correctAnswer) {
      ...
      if (newCombo > 1 && finalDamage > 0) addMessage(`¡Golpe en racha x${newCombo}!`);
  ...
  const handleOpenJarvis = () => {
    if (gameState === 'BATTLE' && isPlayerTurn) {
        ...
        addMessage(`¡El enemigo atacó mientras mirabas la Pokédex/J.A.R.V.I.S! Pierdes ${damage} PS.`);
        setShakePlayer(true);
        setFlashPlayer(true);
    ...
    return (
      <div className="w-full">
        <div className="flex justify-between items-end mb-1">
          <span className="text-[10px] sm:text-xs font-black uppercase text-slate-800 tracking-wider bg-slate-200 px-2 rounded-tl-lg rounded-br-lg border border-slate-400 shadow-sm">{label}</span>
          <span className="text-[9px] sm:text-[10px] font-bold text-slate-600">{current} / {max}</span>
        </div>
        <div className="h-2.5 sm:h-3 w-full bg-slate-300 rounded-full overflow-hidden border-2 border-slate-700 shadow-inner">
          <div className={`h-full ${barColor} transition-all duration-500 ease-out`} style={{ width: `${percent}%` }} />
        </div>
      </div>
    );
  };
      ...
      <div className="min-h-screen bg-slate-800 flex items-center justify-center p-4 font-sans">
        <div className="max-w-md w-full bg-slate-100 rounded-3xl border-[12px] border-slate-700 p-8 text-center shadow-2xl relative overflow-hidden">
          <h1 className="text-4xl font-black mb-2 text-slate-800 uppercase tracking-widest drop-shadow-md">
            Pokémon<br/><span className="text-red-600 text-2xl">Edición Vengadores</span>
          </h1>
          ...
          <p className="mb-8 text-slate-700 font-medium leading-relaxed bg-white p-4 rounded-xl border-4 border-slate-300 shadow-inner text-sm">
            ¡Bienvenido a la región S.H.I.E.L.D.! Calcula áreas, volúmenes y capacidades para derrotar a los villanos. Memoriza las fórmulas o el profesor J.A.R.V.I.S. te cobrará el precio.
          </p>
          <button 
    ...
    return (
      <div className="min-h-screen bg-slate-800 flex items-center justify-center p-4 font-sans">
        <div className="max-w-2xl w-full bg-slate-100 rounded-3xl border-[12px] border-slate-700 p-8 text-center shadow-2xl">
          <h2 className="text-2xl font-black mb-6 text-slate-800 uppercase tracking-widest">Elige a tu Compañero</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
            {HEROES.map((hero) => (
              ...
              >
                ...
                <span className="text-slate-800 font-black text-sm uppercase">{hero.name}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="min-h-screen bg-slate-800 flex items-center justify-center p-4 font-sans">
        <div className="max-w-md w-full bg-slate-100 rounded-3xl border-[12px] border-slate-700 p-8 text-center shadow-2xl relative overflow-hidden">
          ...
          <div className="mb-8 text-slate-700 font-medium leading-relaxed bg-white p-4 rounded-xl border-4 border-slate-300 shadow-inner text-sm">
            {selectedHero?.name} no tiene más energía para luchar. ¡Sal corriendo al centro de curación y estudia las fórmulas de geometría!
          </div>
          <button 
            onClick={startHeroSelection}
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden">
          ...
          {[...Array(80)].map((_, i) => (
            <div
              ...
              style={{
                ...
                top: `-10%`,
                backgroundColor: ['#ef4444', '#3b82f6', '#eab308', '#22c55e', '#a855f7', '#06b6d4'][Math.floor(Math.random() * 6)],
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="mb-8 text-slate-800 font-medium leading-relaxed bg-slate-100 p-6 rounded-2xl border-4 border-yellow-500 shadow-[0_0_30px_rgba(250,204,21,0.4)] text-center max-w-lg relative">
            <p className="text-2xl font-black text-blue-700 mb-2 uppercase">¡{selectedHero?.name} es Leyenda!</p>
            <p className="text-sm font-bold text-slate-700">Has derrotado a Thanos y te has convertido en el maestro indiscutible de Geometría y Medida. ¡El universo está a salvo gracias a tus matemáticas!</p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-2 sm:p-4 font-sans select-none">
          ...
          <div className="bg-red-600 border-8 border-slate-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="bg-red-700 p-4 flex justify-between items-center border-b-4 border-slate-800">
              <div className="flex items-center gap-2">
                 <div className="w-8 h-8 rounded-full bg-blue-400 border-4 border-white shadow-inner flex items-center justify-center"><span className="text-lg">📖</span></div>
                 <h3 className="text-white font-black tracking-widest">Pokédex J.A.R.V.I.S.</h3>
              </div>
              <button onClick={() => setShowJarvis(false)} className="text-white font-bold bg-slate-800 px-3 py-1 rounded-full hover:bg-slate-700 border-2 border-white">❌</button>
            </div>
            <div className="p-4 bg-slate-100 m-4 rounded-xl border-4 border-slate-800 shadow-inner space-y-3 text-sm max-h-[60vh] overflow-y-auto">
              <div className="bg-white p-3 rounded-lg border-2 border-slate-300">
                <h4 className="text-orange-600 font-black mb-1 uppercase">Áreas 2D</h4>
                <p><b>Cuadrado:</b> A = Lado × Lado</p>
                <p><b>Rectángulo / Paralelogramo:</b> A = Base × Altura</p>
                <p><b>Triángulo:</b> A = (Base × Altura) / 2</p>
                <p><b>Círculo:</b> A = π × r²</p>
              </div>
              <div className="bg-white p-3 rounded-lg border-2 border-slate-300">
                <h4 className="text-blue-600 font-black mb-1 uppercase">Volúmenes Básicos</h4>
                <p><b>Cubo:</b> V = Arista³</p>
                <p><b>Prisma:</b> V = Largo × Ancho × Altura</p>
              </div>
              <div className="bg-white p-3 rounded-lg border-2 border-slate-300">
                <h4 className="text-purple-600 font-black mb-1 uppercase">Volúmenes Complejos</h4>
                <p><b>Cilindro:</b> V = π × r² × Altura</p>
                <p><b>Pirámide:</b> V = (Área base × Altura) / 3</p>
                <p><b>Cono:</b> V = (π × r² × Altura) / 3</p>
                <p><b>Esfera:</b> V = (4/3) × π × r³</p>
              </div>
              <div className="bg-white p-3 rounded-lg border-2 border-slate-300">
                <h4 className="text-green-600 font-black mb-1 uppercase">Equivalencias</h4>
                <p><b>Capacidad:</b> 1 Litro = 1000 mL | 1 m³ = 1000 Litros</p>
                <p><b>Masa:</b> 1 dm³ agua = 1 Litro = 1 kg</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full max-w-3xl bg-slate-800 rounded-t-3xl rounded-b-xl border-[16px] border-slate-700 shadow-2xl overflow-hidden flex flex-col h-[95vh] min-h-[650px] max-h-[900px] relative">
        ...
        <div className={`flex-[1.5] relative overflow-hidden transition-colors duration-1000 bg-slate-200 border-b-[8px] border-slate-800`}>
          
          {currentEnemy?.bgImg && (
             <div className="absolute inset-0 bg-cover bg-center opacity-60 z-0" style={{ backgroundImage: `url(${currentEnemy.bgImg})` }} />
          )}
          <div className="absolute inset-0 bg-blue-900/20 pointer-events-none z-0 mix-blend-multiply"></div>
          
          {flashEnemy && <div className="absolute inset-0 bg-red-500/50 z-30 pointer-events-none transition-opacity"></div>}
          {flashPlayer && <div className="absolute inset-0 bg-red-600/50 z-30 pointer-events-none transition-opacity"></div>}
          ...
          <div className="absolute top-4 left-2 sm:left-6 z-20 w-44 sm:w-60">
             <div className="bg-white/95 p-2 sm:p-3 rounded-xl border-4 border-slate-800 shadow-[4px_4px_0_rgba(15,23,42,0.5)] rounded-br-3xl">
                <div className="flex justify-between items-baseline mb-2 border-b-2 border-slate-200 pb-1">
                   <h2 className="font-black text-[11px] sm:text-sm uppercase text-slate-800 truncate pr-2">{currentEnemy?.name}</h2>
                   <span className="text-[10px] sm:text-xs font-bold text-slate-500">Lv{currentEnemy?.level}</span>
                </div>
                <PokeHealthBar current={currentEnemy?.hp} max={currentEnemy?.maxHp} label="HP" />
             </div>
          </div>

          {/* Avatar Enemigo (Top Right) */}
          <div className="absolute top-16 right-2 sm:top-8 sm:right-12 z-30">
             <div className={`transition-all duration-100 ${shakeEnemy ? 'translate-x-3 -translate-y-3 brightness-150' : ''}`}>
               <Avatar isEnemy={true} src={currentEnemy?.img} alt={currentEnemy?.name} fallback={currentEnemy?.fallback} className="w-24 h-24 sm:w-36 sm:h-36 rounded-full border-4 border-slate-800 shadow-xl"/>
             </div>
          </div>
          ...
          <div className="absolute bottom-6 left-4 sm:left-12 z-20">
             <div className={`transition-all duration-100 ${shakePlayer ? '-translate-x-3 translate-y-3 brightness-150' : ''}`}>
                <Avatar isEnemy={false} src={selectedHero?.img} alt={selectedHero?.name} fallback={selectedHero?.fallback} className="w-24 h-24 sm:w-36 sm:h-36 rounded-full border-4 border-slate-800 shadow-xl"/>
             </div>
          </div>
          ...
          <div className="absolute bottom-4 right-2 sm:right-6 z-30 w-48 sm:w-64">
             <div className="bg-white/95 p-2 sm:p-3 rounded-xl border-4 border-slate-800 shadow-[4px_4px_0_rgba(15,23,42,0.5)] rounded-tl-3xl">
                <div className="flex justify-between items-baseline mb-2 border-b-2 border-slate-200 pb-1 pl-4">
                   <h2 className="font-black text-[11px] sm:text-sm uppercase text-slate-800 truncate pr-2">{selectedHero?.name}</h2>
                   <span className="text-[10px] sm:text-xs font-bold text-slate-500">Lv{heroLevel}</span>
                </div>
                <PokeHealthBar current={playerHp} max={maxPlayerHp} label="HP" />
                <div className="mt-1.5">
                  <PokeHealthBar current={energy} max={100} label="EN" colorClass="bg-blue-400" />
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="flex-1 bg-slate-300 p-2 sm:p-4 flex flex-col relative">
          ...
          {question ? (
             ...
             <div className="h-full bg-slate-800 border-4 border-slate-900 rounded-xl p-2 sm:p-4 shadow-inner flex flex-col animate-in slide-in-from-bottom-4">
                <div className="bg-slate-100 text-slate-800 p-2 sm:p-3 rounded-lg border-2 border-slate-400 text-xs sm:text-sm font-black text-center mb-2 shadow-inner flex items-center justify-center min-h-[3rem]">
                  {question.text}
                </div>
                
                {/* Barra de Tiempo */}
                <div className="w-full h-3 bg-slate-600 mb-3 rounded-full overflow-hidden border-2 border-slate-900 relative">
                  <div className="absolute left-0 top-0 h-full bg-yellow-400 transition-all duration-150 ease-linear" style={{ width: `${timeLeft}%` }} />
                </div>

                <div className="grid grid-cols-2 gap-2 flex-1">
                  {question.options.map((opt, idx) => (
                    <button key={idx} onClick={() => handleAnswer(opt)} className="bg-white hover:bg-slate-200 border-4 border-slate-400 hover:border-blue-500 rounded-lg font-black text-slate-800 shadow-[0_4px_0_rgba(148,163,184,1)] active:shadow-none active:translate-y-1 transition-all flex items-center justify-center text-sm sm:text-base uppercase">
                      {opt} {question.unit}
                    </button>
                  ))}
                </div>
            ...
            <div className="h-full bg-white border-8 border-double border-slate-800 rounded-xl shadow-inner flex flex-col sm:flex-row p-2 sm:p-3 gap-2">
              ...
              <div className="flex-1 overflow-y-auto pr-2">
                 <div className="flex items-center justify-between mb-2 pb-1 border-b-2 border-slate-200">
                    <span className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase">Registro de Batalla</span>
                    <button onClick={handleOpenJarvis} className="flex items-center gap-1 bg-blue-100 hover:bg-blue-200 text-blue-800 px-2 py-1 rounded-full border-2 border-blue-300 transition-colors text-[9px] sm:text-[10px] font-black uppercase">
                        <span className="text-xs">📖</span> Pokédex J.A.R.V.I.S.
                    </button>
                 </div>
                 {messages.map((msg, i) => (
                   <div key={i} className={`flex items-start gap-1 sm:gap-2 mb-1.5 ${i === messages.length - 1 ? 'text-slate-800 font-black' : 'text-slate-500 font-medium'} text-xs sm:text-sm leading-snug`}>
                     <span className="mt-0.5 shrink-0 text-slate-800 text-xs">▶️</span>
                     <p dangerouslySetInnerHTML={{ __html: msg.replace(/J\.A\.R\.V\.I\.S\.:/g, '<strong class="text-blue-600">J.A.R.V.I.S.:</strong>').replace(/Fórmula correcta:/g, '<strong class="text-red-600">Fórmula correcta:</strong>') }}></p>
                   </div>
                 ))}
                 {/* Mensaje principal de turno */}
                 {isPlayerTurn && (
                   <p className="font-black text-slate-800 text-sm sm:text-base mt-2 ml-4 animate-pulse">¿Qué debería hacer {selectedHero?.name}?</p>
                 )}
              </div>
                ...
                <div className="w-full sm:w-1/2 md:w-2/5 shrink-0 flex flex-col gap-2 border-t-4 sm:border-t-0 sm:border-l-4 border-slate-200 pt-2 sm:pt-0 sm:pl-2">
                  <div className="grid grid-cols-2 gap-2 flex-1">
                    {selectedHero?.attacks.map((attack, index) => {
                      const bgColors = ['bg-blue-100 hover:bg-blue-200 border-blue-400 text-blue-900', 'bg-purple-100 hover:bg-purple-200 border-purple-400 text-purple-900', 'bg-red-100 hover:bg-red-200 border-red-400 text-red-900'];
                      return (
                        <button key={index} onClick={() => handleAttackClick(attack.diff, attack.name, attack.dmg)} className={`${bgColors[index]} rounded-lg border-b-4 active:border-b-0 active:translate-y-1 transition-all p-1 sm:p-2 flex flex-col items-center justify-center`}>
                          <span className="font-black uppercase text-[9px] sm:text-[11px] leading-tight text-center">{attack.name}</span>
                        </button>
                      )
                    })}
                    <button onClick={handleHealClick} disabled={playerHp >= maxPlayerHp} className="bg-green-100 hover:bg-green-200 border-green-400 text-green-900 border-b-4 active:border-b-0 active:translate-y-1 rounded-lg transition-all p-1 sm:p-2 flex flex-col items-center justify-center disabled:opacity-50 disabled:active:border-b-4 disabled:active:translate-y-0">
                      <span className="font-black uppercase text-[9px] sm:text-[11px] leading-tight text-center">Regenerar</span>
                    </button>
                  </div>
                  {/* Botón Ultimate */}
                  {energy >= 100 && (
                    <button onClick={() => handleAttackClick(3, '💥 IMPACTO CÓSMICO 💥', 200, true)} className="w-full bg-yellow-400 hover:bg-yellow-300 text-slate-900 font-black uppercase text-xs sm:text-sm py-2 rounded-lg border-b-4 border-yellow-600 active:border-b-0 active:translate-y-1 transition-all flex flex-col items-center justify-center animate-pulse">
...
// COMPONENTE CONTENEDOR PARA ASEGURAR QUE TAILWIND CARGA ANTES DE RENDERIZAR
export default function App() {
  ...
  if (!stylesLoaded) {
    return (
      <div style={{ display: 'flex', height: '100vh', width: '100vw', backgroundColor: '#1e293b', color: '#38bdf8', justifyContent: 'center', alignItems: 'center', fontFamily: 'monospace' }}>
        <h2>Iniciando sistemas S.H.I.E.L.D... (Cargando estilos)</h2>
      </div>
    );
  }
